...
namespace Maze
{
    ...
    /// <typeparam name="MazeNode"><![CDATA[IComparable<T> type of item in the heap]]>.</typeparam>
    public class BinaryHeap<MazeNode> 
    {
        // Constants
        private const int DEFAULT_SIZE = 4;
        // Fields
        private MazeNode[] _data = new MazeNode[DEFAULT_SIZE];
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                {
                    MazeNode[] temp = new MazeNode[_capacity];
        ...
        private BinaryHeap(MazeNode[] data, int count)
        {
            Capacity = count;
            _count = count;
        ...
        {
            this._count = 0;
            _data = new MazeNode[_capacity];
        }

        /// Adds a key and value to the heap.
        /// <param name="item">The item to add to the heap.</param>
        public void Add(MazeNode item)
        {
            if (_count == _capacity)
            {
                Capacity *= 2;
        ...
        public MazeNode Remove()
        {
            ...
            _data[_count] = default(MazeNode); //Clears the Last Node
            DownHeap();
            return v;
        }
        private void UpHeap()
        ...
        public BinaryHeap<MazeNode> Copy()
        {
            return new BinaryHeap<MazeNode>(_data, _count);
        }
        ...
        public bool Contains(MazeNode item)
        {
            EnsureSort();
            return Array.BinarySearch<MazeNode>(_data, 0, _count, item) >= 0;
        }
        /// <summary>
        ...
        public void CopyTo(MazeNode[] array, int arrayIndex)
        {
            EnsureSort();
        ...
        public bool Remove(MazeNode item)
        {
            EnsureSort();
            int i = Array.BinarySearch<MazeNode>(_data, 0, _count, item);
            if (i < 0) return false;
            Array.Copy(_data, i + 1, _data, i, _count - i);
            _data[_count] = default(MazeNode);
            _count--;
            return true;
        }
